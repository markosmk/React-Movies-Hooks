...
interface ListTvShowsProps {
  listing: Media[];
  isLoading: boolean;
}
export const ListTvShows = ({ listing }: ListTvShowsProps) => {
  return (
    <>
              ...
              >
                ...
                <div className="absolute left-4 right-10 bottom-4 z-10">
                  <div className="flex items-center space-x-3">
                    ...
                    <h2 className="inline-flex text-2xl leading-none text-white font-bold tracking-wide">{title}</h2>
                  </div>
                  ...
                  <p className="text-xs sm:text-sm leading-5 sm:leading-6 text-slate-300">{overview}</p>
                </div>
                ...
                <div className="relative block overflow-hidden group-hover:before:w-5/6 before:w-full before:absolute before:inset-0 before:bg-gradient-to-r before:from-black before:to-transparent before:z-0 before:transition-all before:duration-300">
                  <img src={backdrop} alt={title} className="w-full block transition-transform duration-300" />
                </div>
              </Link>
            );
          })}
